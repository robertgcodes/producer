...
const feedUrlMappings: Record<string, string> = {
  ...
  // YouTube Channels (these need special handling)
  'Watching the Watchers YouTube Channel': 'YOUTUBE:https://www.youtube.com/@WatchingTheWatchers',
  'YouTube Fox News': 'YOUTUBE:https://www.youtube.com/@FoxNews',
  'Homeland Security YouTube': 'YOUTUBE:https://www.youtube.com/@DHSgov',
  'DHS YouTube': 'YOUTUBE:https://www.youtube.com/@DHSgov',
  ...
  // For Google News searches (these need special handling)
  '\"trump litigation\"': 'GOOGLENEWS:trump litigation',
  '\"Zohran Mamdani\"': 'GOOGLENEWS:Zohran Mamdani',
  '\"u.s. supreme court\"': 'GOOGLENEWS:u.s. supreme court',
};
...
async function fixFeedUrls() {
  console.log('Starting feed URL fix...\n');
  ...
  try {
    ...
    let fixedCount = 0;
    let skippedCount = 0;
    let unknownFeeds: string[] = [];
    
    for (const feed of feeds) {
      if (feed.url === '/api/rss' || !feed.url || feed.url.startsWith('/api/')) {
        console.log(`Processing: ${feed.title}`);
        ...
        if (newUrl) {
          ...
          if (newUrl.startsWith('YOUTUBE:')) {
            feedType = 'youtube';
            newUrl = newUrl.replace('YOUTUBE:', '');
          } else if (newUrl.startsWith('GOOGLENEWS:')) {
            feedType = 'googlenews';
            ...
            console.log(`  ✓ Fixed as Google News search: "${newUrl}"`);
            fixedCount++;
            continue;
          }
          ...
          console.log(`  ✓ Fixed: ${newUrl} (type: ${feedType})`);
          fixedCount++;
        } else {
    ...
    console.log('\n=== Summary ===');
    console.log(`Fixed: ${fixedCount} feeds`);
    console.log(`Skipped: ${skippedCount} feeds (already have valid URLs)`);
    console.log(`Unknown: ${unknownFeeds.length} feeds\n`);
    
    if (unknownFeeds.length > 0) {
      console.log('Feeds without mappings:');
      unknownFeeds.forEach(title => console.log(`  - ${title}`));
      console.log('\nThese feeds will need to be updated manually or deleted.');
    }
    
    console.log('\nDone! Your feeds should now refresh properly.');
